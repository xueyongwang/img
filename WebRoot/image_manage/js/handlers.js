...
function initSwfupload(url, sizeLimit, types, typsdesc, uploadLimit) {
	...
	var settings = {
		...
		flash_url : contextPath + "/image_manage/swf/swfupload.swf",
		...
		// Button Settings
		button_image_url : contextPath
				+ "/image_manage/images/select-files-zh_CN.png",
	...
	try {
		...
		var message = '<div id="divAlternateContent" style="background-color: #FFFF66; text-align:center;">' + '<font color="red">工具加载失败,请刷新再试!</div>';
...
function init(uploadLimit, sizeLimit) {
	...
	var uploadbody = '<div id="uploadBody" style="position:absolute;left:15%;top:30%;" class="ke-dialog-upload">'
...
function swfUploadLoadFailed() {
	var message = '<div id="divAlternateContent" style="background-color: #FFFF66; text-align:center;">' + '<a href="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash" target="_blank"><font color="red">请安装或者升级您的Flash插件!</font></a></div>';
	addErrorMsg(message, false);
}
...
function addReadyFileInfo(fileid, fileName, message) {
	var photoDiv = '<div class="ke-inline-block ke-item" id="'
			...
			+ '	<img src="'
			+ contextPath
			+ '/image_manage/images/image.png" class="ke-img" data-status="-1" width="80" height="80" />'
